...
const labelKwartaalKort = (code) => `K${code.slice(6)} '${code.slice(2, 4)}`
const labelKwartaalVol = (code) => `${code.slice(0, 4)} kwartaal ${parseInt(code.slice(6), 10)}`
...
function pct(nu, eerder) {
  return eerder ? ((nu - eerder) / eerder) * 100 : 0
}

function leidSignalenAf(trend, regios, flow) {
  const s = []
  if (flow.length) {
  ...
  if (regios.length) {
    ...
    const regiosZonderNL = regios.filter(r => r.regio !== 'Heel Nederland')
    if (regiosZonderNL.length) {
      const grootste = [...regiosZonderNL].sort((a, b) => b.totaal - a.totaal)[0]
      s.push({
        id: 'regio-totaal',
        niveau: 'kans',
...
async function buildData() {
  ...
  const soortFilter = encodeURIComponent("substringof('2025MM',Perioden) or substringof('2026MM',Perioden) or substringof('2027MM',Perioden)")
  const soortSelect = 'Perioden,BijstandsuitkeringenTotDeAOWLeeftijd_12,NietSeizoengecorrigeerd_8,WajongUitkeringen_3,TotaalWIAUitkeringen_5,WAOUitkeringen_2,IVAUitkeringen_6,WGAUitkeringen_7'
  ...
  const flowFilter = encodeURIComponent("Geslacht eq 'T001038' and Leeftijd eq '10000' and Herkomstland eq 'T001040' and GeboortelandOuders eq 'T001638' and (substringof('2023',Perioden) or substringof('2024',Perioden) or substringof('2025',Perioden) or substringof('2026',Perioden) or substringof('2027',Perioden))")
  ...
  const [soortRows, periodenRows, flowRows] = await Promise.all([
    haal(soortUrl), haal(periodenUrl), haal(flowUrl)
  ])

  const bijstandFlow = flowRows
  ...
  const kpi = (id, label, nu, eerder, toelichting) => ({ id, label, waarde: Math.round(nu * 1000), vorigePeriode: Math.round(eerder * 1000), eenheid: '', richting: 'slecht', toelichting })
  const ao = (r) => (r.TotaalWIAUitkeringen_5 ?? 0) + (r.WAOUitkeringen_2 ?? 0) + (r.WajongUitkeringen_3 ?? 0)
  ...
  const data = { peilmaandSoort, peilmaandRegio, peilkwartaalFlow, kpis, trend, bijstandFlow, soortVerdeling, aoRegeling, regios, signalen }
