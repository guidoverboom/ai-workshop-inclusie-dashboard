...
export function RegioTabel({ data, geselecteerdeRegios, onToggleRegio }: RegioTabelProps) {
        ...
        <tbody>
          {rijen.map((r) => {
            ...
            return (
              <tr
                key={r.regio}
                onClick={() => onToggleRegio(r.id)}
                className={`border-b cursor-pointer group transition-colors ${
                  isChecked ? 'bg-indigo-50/60' : 'hover:bg-slate-50/80'
                } ${r.regio === 'Heel Nederland' ? 'border-slate-300 shadow-sm z-10 relative' : 'border-slate-100'}`}
              >
